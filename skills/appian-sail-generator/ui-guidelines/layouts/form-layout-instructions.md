...
#### a!headerTemplateImage
A header template that prominently features a banner image — typically used when the form represents an entity with a visual identity (a product, a property, a person). The image spans the header band; the title sits over or beside it depending on configuration.

```sail
a!formLayout(
  titleBar: a!headerTemplateImage(
    title: "Vintage Typewriter — Olivetti Lettera 32",
    secondaryText: "Catalog Entry · Restoration in progress",
    image: a!webImage(
      source: "https://example.com/typewriter-hero.jpg",
      altText: "Photo of an Olivetti Lettera 32 typewriter"
    ),
    titleColor: "#FFFFFF",
...
- `title`: Main heading text
- `secondaryText`: Subtitle or description
- `image`: Required image (`a!webImage`, `a!documentImage`, or `a!userImage`)
- `titleColor`: Color for main title (hex or enumeration)
- `secondaryTextColor`: Color for subtitle

**When to use vs the other templates:**
| Goal | Template |
|---|---|
| Plain title bar | `a!headerTemplateSimple` |
| Solid coloured banner with title | `a!headerTemplateFull` |
| Banner with a photo / hero image | `a!headerTemplateImage` |
| Vertical sidebar on desktop, header on mobile | `a!sidebarTemplate` |
