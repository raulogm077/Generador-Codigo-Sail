...
- **titleBar**: Text or header template for wizard title (see "Title Bar Templates" below)
...
### Title Bar Templates
The `titleBar` parameter accepts a plain text string or one of four header templates. The choice affects the visual weight of the title and what surrounding content (image, sidebar, accents) accompanies it.

| Template | Use case |
|---|---|
| `a!headerTemplateSimple` | Plain title, content-width aligned. Default for most wizards. |
| `a!headerTemplateFull` | Solid-colour banner spanning the screen. For prominent or branded wizards. |
| `a!headerTemplateImage` | Banner with a hero image. For wizards about a specific entity with a visual identity (product setup, employee onboarding, property listing). |
| `a!sidebarTemplate` | Sidebar on desktop, collapses to header on mobile. For wizards where contextual info should stay visible alongside the steps. |

```sail
/* Hero-image titleBar example */
a!wizardLayout(
  titleBar: a!headerTemplateImage(
    title: "New Employee Onboarding",
    secondaryText: "Welcome to the team",
    image: a!webImage(source: "https://example.com/onboarding-banner.jpg"),
    titleColor: "#FFFFFF",
...
Full parameter documentation for each template lives in `ui-guidelines/layouts/form-layout-instructions.md` § Header Templates (the templates are identical between FormLayout and WizardLayout).
