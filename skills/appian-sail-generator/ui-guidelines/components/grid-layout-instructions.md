...
- **allowRowReordering** (Boolean): Enables drag-and-drop row reordering (default: false)
- **rowOrderTooltip**: Tooltip config for the reorder handle (official example uses `rowOrderTooltip: true`)
- **rowOrderData**: The local variable or rule input holding the grid data list to update on reorder
- **rowOrderField**: Integer field storing the order index — record type: `recordType!X.fields.orderIndex`; CDT/map: the field name as a string (`"orderIndex"`). Cannot be a related-record field.

⛔ **`rowDeletions` / `a!gridRowDeletion()` DO NOT EXIST in Appian.** Earlier versions of this guide documented them by mistake. Row deletion in an editable grid is done with a richText trash-icon link column that `remove()`s the item from the source list (see the example above), or — for record-backed read-only grids — with the official "Delete Rows in a Grid" recipe (selection + a REMOVE button that filters out selected IDs).
...
### Row Reordering with allowRowReordering (verified against official docs)

Let users drag-and-drop rows; the order index is written to a dedicated Integer field:
...
a!gridLayout(
  ...
  /* rowOrderData: the SAME variable that backs the grid rows */
  rowOrderData: local!items,
  /* Record type: field reference; CDT/map: the field name as a string */
  rowOrderField: "orderIndex"
)
```

Requirements: the order field must be `Number (Integer)`, cannot live in a related record, and `rowOrderData` must point at the exact variable/rule input used for the rows. Provide an accessible alternative (e.g. up/down arrow links) for users who can't drag.

> ⛔ Do NOT use `rowDeletions:` or `a!gridRowDeletion()` — that function does not exist in Appian (it was documented here by mistake in older versions of this guide). Delete rows with the trash-icon link pattern shown earlier.
...
- [ ] ✅ Delete via richText trash-link column configured (⛔ `rowDeletions` does not exist)
